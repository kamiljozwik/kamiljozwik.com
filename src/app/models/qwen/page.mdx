...
Built upon the proven [transformer architecture](https://www.youtube.com/watch?v=ZXiruGOCn9s), the majority of Qwen models are released under the permissive **Apache 2.0 license**, readily available on platforms like [Hugging Face](https://huggingface.co/Qwen). This open approach allows us to experiment, customize, and deploy these models locally or within our own infrastructure, complementing the API access provided via [Alibaba Cloud](https://www.alibabacloud.com/).
...
At the heart of the family lies [Qwen2.5](https://github.com/QwenLM/Qwen2.5), a series of powerful and efficient text-based LLMs. It’s a dense, decoder-only transformer-based LLM with improved capabilities over `Qwen2`. These serve as the foundation for many specialized models and are excellent general-purpose tools.

Here are some key features of `Qwen2.5`:

- Models come in a wide array of sizes: **0.5B, 1.5B, 3B, 7B, 14B, 32B**, and **72B** parameters. You can find them on [HuggingFace](https://huggingface.co/collections/Qwen/qwen25-66e81a666513e518adb90d9e).
...
- **Multilingual:** Supports numerous _programming_ languages, covering mainstream ones like Python, Java, C++, JavaScript, etc. Natural language prompts are best handled in English and Chinese.
...
- Offered in **1.5B**, **7B**, and **72B** parameters, plus a **72B** [reward model](https://huggingface.co/Qwen/Qwen2.5-Math-RM-72B) used for training.
- **Context window** has **128k** tokens.
...
- **Proprietary** ([API access](https://www.alibabacloud.com/help/en/model-studio/model-api-reference) via Alibaba Cloud). The model weights are [not publicly released](https://qwenlm.github.io/blog/qwen2.5-max/).
...
Building upon the success of its predecessors, Alibaba introduced [Qwen3](https://huggingface.co/collections/Qwen/qwen3-67dd247413f0e2e4f653967f), the latest generation of large language models in the Qwen series. Announced [late April 2025](https://qwenlm.github.io/blog/qwen3/), `Qwen3` offers a comprehensive suite of dense and `MoE` models, delivering some advancements in reasoning, instruction-following, agent capabilities, and multilingual support.

- **Architecture & model range:** `Qwen3` continues the Transformer-based architecture but introduces significant enhancements and a diverse lineup:
  - **Dense models:** Six open-source dense models were released, ranging in size from **0.6B, 1.7B, 4B, 8B, 14B, to 32B**.
  - **MoE models:** Two powerful and efficient open-source `MoE` models:
    - `Qwen3-30B-A3B`: A compact `MoE` model with **30B total parameters**, but only **3B active** during inference, offering good performance for its computational footprint.
    - `Qwen3-235B-A22B`: The flagship `MoE` model with **235B total parameters** and **22B active**, designed to compete at the highest level.
- **Context window:** Context length varies across the models:
  - The smaller dense models (0.6B, 1.7B, 4B) support a **32K token** context.
  - Larger dense models (8B, 14B, 32B) and both MoE models (30B, 235B) support an extended **128K token** context length.
- **Modality:** `Qwen3` models are currently **text-only**, focusing on text input and text output (including code). They do not natively support image or audio modalities in this release series.
- **Multilingual support:** `Qwen3` significantly expands its linguistic reach, supporting **119 languages and dialects**.
- **Function calling / tool use:** Agentic capabilities have been notably improved. `Qwen3` models are optimized for coding and agentic tasks, with strengthened support for tool calling and MCP.
- **Hybrid thinking modes:** A key innovation in `Qwen3` is the introduction of hybrid thinking modes:
  1.  **Thinking mode:** The model takes more time to reason step-by-step before delivering the final answer, ideal for complex problems requiring deeper thought (similar to `Chain-of-Thought`).
  2.  **Non-thinking mode:** Provides quick, near-instant responses, suitable for simpler questions where speed is prioritized over depth.
      This allows users to configure a "thinking budget" (in tokens) to balance response quality with latency and computational cost.
- **Use Cases:** `Qwen3` models are suitable for a wide range of applications, building on the strengths of Qwen2.5 but with enhanced performance, particularly in coding, math, and multilingual scenarios. Their improved agentic capabilities make them excellent choices for building sophisticated AI agents and tool-using applications. The Hybrid Thinking feature adds a layer of control for developers needing to balance complexity and speed.

You can also check out this nice video overview of `Qwen3` [here](https://www.youtube.com/watch?v=K3MlgNf__bc).
...
[Qwen Chat](https://chat.qwen.ai/) is an UI chat interface for the Qwen family of models. Chat allows users to interact with the models in a conversational manner, making it easy to test and explore their capabilities for free.

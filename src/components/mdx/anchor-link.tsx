...
const Wrapper = ({ children, className }: PropsWithChildren<HTMLProps<HTMLSpanElement>>) => <span className={cn("inline-flex gap-1 items-center mx-1", className)}>{children}</span>
...
export const AnchorLink = ({
  ...
  switch (true) {
    case href?.startsWith("#"):
      return (
        <a href={href} {...rest} className="font-normal underline-offset-3">
          {children}
        </a>
      );
    ...
    case href?.includes("youtube.com"):
      return (
        <Wrapper>
          <YouTubeLogo />
          <ExternalLink href={href} className="font-normal underline-offset-3">
            {children}
          </ExternalLink>
        </Wrapper>
      );

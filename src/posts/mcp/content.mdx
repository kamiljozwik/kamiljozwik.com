...
If all the above sounds to you like **"just another API"**, here is [another video](https://www.youtube.com/watch?v=7j1t3UZA1TY) (IBM again) explaining key differences between APIs and MCP.
...
- [Pipedream](https://mcp.pipedream.com/) - servers
...
You can also (of course) ask your AI coding assistant to [build an MCP server in 5 prompts](https://www.youtube.com/watch?v=FRogt98OF80). The MCP creators made use of [llms.txt files](/posts/llms-txt) and provide us [the whole documentation](https://modelcontextprotocol.io/llms-full.txt) needed for LLM to get things done.

And for a really lazy developers, you can just use [MCPify](https://mcpify.ai/)

What a time to build amazing things 🤖

## Development docs

I am pretty sure, that for developers, one of the most common use cases for MCP is to build or use a server that can reach for up-to-date documentation.

For that purpose, you can use [Context7 MCP Server](https://github.com/upstash/context7) to pull up-to-date, version-specific documentation and code examples for frameworks like Next.js, MongoDB, .NET etc. straight from the source — and places them directly into your prompt.

Another useful tool is [gitmcp](https://gitmcp.io/), which allows to instantly create a remote MCP server for any GitHub repository.

## Hosting

Based on my experience and research, it looks like Cloudflare is one of the best and the easiest places to host your MCP servers.

Cloudflare's team is doing a really great job in providing documentation [[1]](https://developers.cloudflare.com/agents/guides/remote-mcp-server/), [[2]](https://developers.cloudflare.com/agents/model-context-protocol/), tutorials [[1]](https://blog.cloudflare.com/remote-model-context-protocol-servers-mcp/), [[2]](https://www.youtube.com/watch?v=Pjc8cC8zVRY) and [examples](https://github.com/cloudflare/ai/tree/main/demos/remote-mcp-authless).

Some of the examples go [way beyond 🍕](https://www.youtube.com/watch?v=oyl-UsWXoKU) standard use case, showcasing how universal MCP servers can be.
